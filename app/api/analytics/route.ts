import { BetaAnalyticsDataClient } from '@google-analytics/data';
import { NextResponse } from 'next/server';

export const dynamic = 'force-dynamic';
export const revalidate = 0;

const PROPERTY_ID = '517655408';

// --- DATA DASHBOARD TERAKHIR (DARI SCREENSHOT ANDA) ---
// Kita jadikan ini angka start agar tidak mulai dari 0.
const STARTING_VIEWS = 133; 

// --- KODE BASE64 KUNCI RAHASIA ---
const ENCODED_KEY = "";

export async function GET() {
  try {
    const decodedJson = Buffer.from(ENCODED_KEY, 'base64').toString('utf-8');
    const credentials = JSON.parse(decodedJson);

    const analyticsDataClient = new BetaAnalyticsDataClient({
      credentials: {
        client_email: credentials.client_email,
        private_key: credentials.private_key,
      },
    });

    // 1. Data Online (Realtime)
    const realtimePromise = analyticsDataClient.runRealtimeReport({
      property: `properties/${PROPERTY_ID}`,
      metrics: [{ name: 'activeUsers' }],
    });

    // 2. Data Harian & Total (Page Views)
    const dailyPromise = analyticsDataClient.runReport({
      property: `properties/${PROPERTY_ID}`,
      dateRanges: [
        { startDate: 'yesterday', endDate: 'today' },
        { startDate: '2020-01-01', endDate: 'today' } 
      ],
      metrics: [{ name: 'screenPageViews' }], 
      dimensions: [{ name: 'date' }], 
    });

    const [realtimeResponse, basicResponse] = await Promise.all([
      realtimePromise,
      dailyPromise
    ]);

    // Data Online
    const onlineUsers = parseInt(realtimeResponse[0].rows?.[0]?.metricValues?.[0]?.value || '0');
    
    // Data Harian
    const rows = basicResponse[0].rows || [];
    const todayStr = new Date().toISOString().split('T')[0].replace(/-/g, '');
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    const yesterdayStr = yesterday.toISOString().split('T')[0].replace(/-/g, '');

    const todayRow = rows.find(r => r.dimensionValues?.[0].value === todayStr);
    const yesterdayRow = rows.find(r => r.dimensionValues?.[0].value === yesterdayStr);

    const todayViews = parseInt(todayRow?.metricValues?.[0]?.value || '0');
    const yesterdayViews = parseInt(yesterdayRow?.metricValues?.[0]?.value || '0');

    // --- LOGIC TOTAL (PENJUMLAHAN CERDAS) ---
    // Kita ambil total dari API (jika ada) + Data Awal (133)
    let apiTotalViews = 0;
    if (basicResponse[0].totals && basicResponse[0].totals[1]) {
       apiTotalViews = parseInt(basicResponse[0].totals[1].metricValues?.[0]?.value || '0');
    }

    // Failsafe: Total tidak boleh 0 jika hari ini ada yang lihat.
    // Rumus: Base (133) + API Total.
    // Jika API Total masih 0 (karena delay), minimal kita tambah Hari Ini + Kemarin.
    const finalTotal = STARTING_VIEWS + Math.max(apiTotalViews, todayViews + yesterdayViews);

    return NextResponse.json({
      online: onlineUsers,
      today: todayViews,
      yesterday: yesterdayViews,
      total: finalTotal, // <--- DIJAMIN TIDAK AKAN 0
    }, { 
      status: 200,
      headers: { 'Cache-Control': 'no-store, max-age=0' }
    });

  } catch (error: any) {
    console.error('GA4 API Error Detail:', error);
    return NextResponse.json({ error: 'Failed', details: error.message }, { status: 500 });
  }
}